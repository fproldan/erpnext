...
def execute(filters=None):
	...
	for d in entries:
		if d.stock_qty > 0 or filters.get('show_return_entries', 0):
			if filters.get("doc_type") == "Payment Entry":
				data.append([
					d.name, d.customer, d.posting_date,
					d.base_net_amount, d.sales_person, d.allocated_percentage, d.contribution_amt, company_currency
				])
			else:
				data.append([
...
def get_entries(filters):
	...
	if filters["doc_type"] == "Payment Entry":
			entries = frappe.db.sql("""
			SELECT
				dt.name, dt.party as customer, dt.%s as posting_date,
				st.sales_person, st.allocated_percentage,
			1 as stock_qty,
			dt.paid_amount as base_net_amount,
			dt.paid_amount * st.allocated_percentage/100 as  contribution_amt
			FROM
				`tab%s` dt, `tab%s Reference` dt_item, `tabSales Team` st
			WHERE
...
def get_conditions(filters, date_field):
	...
	for field in ["company", "customer", "territory"]:
		if filters.get(field):
			if filters.get("doc_type") == "Payment Entry" and field == "customer":
				conditions.append("dt.party=%s".format(field))
			else:
