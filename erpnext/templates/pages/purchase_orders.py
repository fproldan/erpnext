...
def get_context(context, **dict_params):
    ...
    frappe.local.form_dict.doctype = 'Purchase Order'
    ...
    context.transaction_date = frappe.local.form_dict.transaction_date
    context.schedule_date = frappe.local.form_dict.schedule_date
    context.nuevo_pacto_entrega = frappe.local.form_dict.nuevo_pacto_entrega
    context.status = frappe.local.form_dict.status
    frappe.local.form_dict.fields = ['name', 'docstatus', 'transaction_date', 'bill_date']
    context.update(get(**frappe.local.form_dict))
    context.template = 'templates/pages/purchase_orders.html'
    context.list_template = 'templates/pages/purchase_orders_list.html'
    

@frappe.whitelist(allow_guest=True)
def get(doctype, company=None, txt=None, transaction_date=None, nuevo_pacto_entrega=None, status=None, schedule_date=None, limit_start=0, limit=20, pathname=None, **kwargs):
    """Returns processed HTML page for a standard listing."""
    limit_start = cint(limit_start)

    raw_result = get_list_data(doctype, company, txt, transaction_date, nuevo_pacto_entrega, status, schedule_date, limit_start, limit=limit + 1, **kwargs)
    ...
    result = []
    row_template = 'templates/pages/purchase_orders_row.html'
    list_view_fields = [df for df in meta.fields if df.fieldname in ['title', 'transaction_date', 'schedule_date', 'status', 'nuevo_pacto_entrega']]
...
@frappe.whitelist(allow_guest=True)
def get_list_data(doctype, company=None, txt=None, transaction_date=None, nuevo_pacto_entrega=None, status=None, schedule_date=None, limit_start=0, fields=None, cmd=None, limit=20, web_form_name=None, **kwargs):
    """Returns processed HTML page for a standard listing."""
    limit_start = cint(limit_start)
    ...
    kwargs = dict(
        doctype=doctype, company=company, txt=txt, transaction_date=transaction_date, nuevo_pacto_entrega=nuevo_pacto_entrega, status=status, schedule_date=schedule_date, filters=filters,
...
def get_transaction_list(doctype, company=None, txt=None, transaction_date=None, nuevo_pacto_entrega=None, status=None, schedule_date=None, filters=None, limit_start=0, limit_page_length=20, order_by="modified", custom=False):
    ...
    transactions = get_list_for_transactions(doctype, txt, filters, limit_start, limit_page_length,
        fields='name', ignore_permissions=ignore_permissions, order_by='modified desc', transaction_date=transaction_date, nuevo_pacto_entrega=nuevo_pacto_entrega, company=company, status=status, schedule_date=schedule_date)
